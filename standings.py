...
async def standings(request: Request, org: str, lg: str, yr: int, sort: Optional[str] = 'Pct', asc: Optional[bool] = False):
    ...
    df = df[(df['Org']==org) & (df['League']==lg) & (df['Year']==yr)].groupby('Team')['wRAAc'].sum().reset_index()
    st = st.merge(df, on='Team', how='left').fillna(0)
    st['wRAAc'] = round(st['wRAAc'],1)
    pit = pit[(pit['Org']==org) & (pit['League']==lg) & (pit['Year']==yr)].groupby('Team')['WAR'].sum().reset_index()
