...
async def draft_view(request: Request):
    ...
    return templates.TemplateResponse('draft.html', {'request':request, 'hitters':h[h['Owner'].isna()], 'owned':h[h['Owner'].notna()], 'owners_df':owners_df, 'roster':roster, 'json':h.set_index('Name').to_json(orient='index')})
